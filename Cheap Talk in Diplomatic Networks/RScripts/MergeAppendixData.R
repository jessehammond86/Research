...
##  - Yearly data on political relevance (1995 - 2001)
##
...
# load('./Data/AnalysisData/yearly_commdata_raw.Rdata')
# load('./Data/AnalysisData/yearly_commdata_gold.Rdata')
...
states <- rownames(yearly_arrays_gold[[1]][,,1])
...
for(this_year in 1:length(years)){
  yearly_commdata_gold[[this_year]] <- data.table(
...
yearly_data <- rbindlist(yearly_commdata_gold)
...
yearly_data[
  is.na(mid_start) & year < 2001
  , mid_start := 0
  ]
yearly_data[
  is.na(mid_highest_intensity) & year < 2001
  , mid_highest_intensit := 0
  ]
...
save(yearly_data, file = './Data/AppendixData/yearly_data.Rdata')
