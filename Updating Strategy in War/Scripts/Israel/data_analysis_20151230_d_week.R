...
pal_casratio <- inputs4[!is.na(inputs5)]
...
# Positive shock on AREA C targeting from CASUALTY RATIOS
armodel2 <- arima(input_data[, list(mean_palevent_area_c)], order = c(4,0,0), xreg = exog)
armodel2
...
########## PAL TARG UP - ISR LOC UP
inputs = xts(input_data[,list(mean_isrevent_city, pal_civtargeting, mean_palevent_area_ab
)], order.by = input_data$week)
VARselect(inputs, lag.max = 8, type = 'both')$selection
...
##### CP detection
# PAL indirect
pal_cp <- e.divisive(as.matrix(inputs$pal_indirect), R = 999)
pal_change_idx <- pal_cp$estimates[c(2,3)]
pal_means <- data.frame('mean' = c(mean(inputs[1:pal_change_idx[1], pal_indirect])
                                   , mean(inputs[pal_change_idx[1]:pal_change_idx[2], pal_indirect])
                                   , mean(inputs[pal_change_idx[2]:nrow(inputs), pal_indirect])))
# ISR heavy arms
isr_cp <- e.divisive(as.matrix(inputs$isr_bigtech), R = 999)
isr_change_idx <- isr_cp$estimates[c(2,3)]
isr_means <- data.frame('mean' = c(mean(inputs[1:isr_change_idx[1], isr_bigtech])
                                   , mean(inputs[isr_change_idx[1]:nrow(inputs), isr_bigtech])))
...
## PAL CP
date_range <- data.frame('from' = inputs$week[pal_change_idx]
                         , 'to' = inputs$week[pal_change_idx+2])

dev.off()
pdf(file = 'palCP.pdf', width = 10, height = 6)
ggplot() +
  geom_line(data = inputs, aes(week, pal_indirect)) +
  geom_rect(data = date_range, aes(xmin = from, xmax = to, ymin = -Inf, ymax = Inf)
            , alpha = 0.5, fill = c('blue')) +
  geom_segment(aes(x=inputs$week[1], xend=inputs$week[pal_change_idx[1]]
                   , y=0.286, yend=0.286), color = 'red') +
  geom_segment(aes(x=inputs$week[pal_change_idx[1]], xend=inputs$week[pal_change_idx[2]]
                   , y=0.398, yend=0.398), color = 'red') +
  geom_segment(aes(x=inputs$week[pal_change_idx[2]], xend=inputs$week[nrow(inputs)]
