...
### Types of location
# Population
data[, mean_palevent_pop := mean(as.numeric(Population[actor1 %in% pal_milactors]), na.rm = T), by = list(date)]
data[, sd_palevent_pop := sd(as.numeric(Population[actor1 %in% pal_milactors]), na.rm = T), by = list(date)]
data[is.na(mean_palevent_pop), mean_palevent_pop := 0]
data[is.na(sd_palevent_pop), sd_palevent_pop := 0]
data[, mean_isrevent_pop := mean(as.numeric(Population[actor1 %in% isr_milactors]), na.rm = T), by = list(date)]
data[, sd_isrevent_pop := sd(as.numeric(Population[actor1 %in% isr_milactors]), na.rm = T), by = list(date)]
data[is.na(mean_isrevent_pop), mean_isrevent_pop := 0]
data[is.na(sd_isrevent_pop), sd_isrevent_pop := 0]

# Israel vs palestine location
data[, mean_palevent_pal := mean(palestine[actor1 %in% pal_milactors], na.rm = T), by = list(date)]
data[is.na(mean_palevent_pal), mean_palevent_pal := mean(data$mean_palevent_pal, na.rm = T)]
data[, mean_isrevent_pal := mean(palestine[actor1 %in% isr_milactors], na.rm = T), by = list(date)]
data[is.na(mean_isrevent_pal), mean_isrevent_pal := mean(data$mean_isrevent_pal, na.rm = T)]

# Distance from border
data[, mean_palevent_bdist := mean(mindist_border[actor1 %in% pal_milactors], na.rm = T), by = list(date)]
data[, sd_palevent_bdist := sd(mindist_border[actor1 %in% pal_milactors], na.rm = T), by = list(date)]
...
data[, mean_isrevent_bdist := mean(mindist_border[actor1 %in% isr_milactors], na.rm = T), by = list(date)]
data[, sd_isrevent_bdist := sd(mindist_border[actor1 %in% isr_milactors], na.rm = T), by = list(date)]
...
### Casualties
data[, paldead_mo := sum(pal_fatali), by = list(date)]
data[, paldead_civ_mo := sum(pal_nonc_1), by = list(date)]
data[, paldead_milt_mo := sum(pal_mili_1), by = list(date)]
data[, paldead_mili_mo := sum(pal_mili_2), by = list(date)]
...
data[, paldead_mil_mo := paldead_milt_mo + paldead_mili_mo, by = list(date)]
data[, paldead_mili_mo := NULL]
data[, paldead_milt_mo := NULL]

data[, isrdead_mo := sum(isr_fatali), by = list(date)]
data[, isrdead_civ_mo := sum(isr_nonc_1), by = list(date)]
data[, isrdead_mil_mo := sum(isr_combat), by = list(date)]
data[is.na(isrdead_mo), isrdead_mo := 0]
data[is.na(isrdead_civ_mo), isrdead_civ_mo := 0]
data[is.na(isrdead_mil_mo), isrdead_mil_mo := 0]

data[, palwound_mo := sum(pal_combin), by = list(date)]
data[, palwound_civ_mo := sum(pal_noncom), by = list(date)]
data[, palwound_mil_mo := sum(pal_milita), by = list(date)]
...
data[, isrwound_civ_mo := sum(isr_noncom), by = list(date)]
data[, isrwound_mil_mo := sum(isr_milita), by = list(date)]
data[, isrwound_pol_mo := sum(isr_police), by = list(date)]
data[, isrwound_mil_mo := isrwound_mil_mo + isrwound_pol_mo, by = list(date)]
...
### Interactions
data[, event := 1.0]
data[, pal_event := sum(event == 1 & (actor1 %in% pal_milactors & interactio %in% violent_events) | (interactio == 'firefight')), by = list(date)]
data[, isr_event := sum(event == 1 & (actor1 %in% isr_milactors & interactio %in% violent_events) | (interactio == 'firefight')), by = list(date)]
data[, mut_event := sum(event == 1 & interactio %in% 'firefight'), by = list(date)]

# Shootings
data[, pal_shooting := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'shooting')), by = list(date)]
data[pal_event > 0, pal_shooting := pal_shooting / pal_event, by = list(date)]
data[, isr_shooting := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'shooting')), by = list(date)]
data[isr_event > 0, isr_shooting := isr_shooting / isr_event, by = list(date)]

# Firefights
data[, pal_firefight := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'firefight')), by = list(date)]
data[pal_event > 0, pal_firefight := pal_firefight / pal_event, by = list(date)]
data[, isr_firefight := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'firefight')), by = list(date)]
data[isr_event > 0, isr_firefight := isr_firefight / isr_event, by = list(date)]

# Shellings
data[, pal_shelling := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'shelling')), by = list(date)]
data[pal_event > 0, pal_shelling := pal_shelling / pal_event, by = list(date)]
data[, pal_smallshelling := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'shelling' & technolo_1 %in% small_arms)), by = list(date)]
data[pal_event > 0, pal_smallshelling := pal_smallshelling / pal_event, by = list(date)]
data[, pal_bigshelling := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'shelling' & technolo_1 %in% big_arms)), by = list(date)]
data[pal_event > 0, pal_bigshelling := pal_bigshelling / pal_event, by = list(date)]

data[, isr_shelling := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'shelling')), by = list(date)]
data[isr_event > 0, isr_shelling := isr_shelling / isr_event, by = list(date)]
data[, isr_smallshelling := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'shelling' & technolo_1 %in% small_arms)), by = list(date)]
data[isr_event > 0, isr_smallshelling := isr_smallshelling / isr_event, by = list(date)]
data[, isr_bigshelling := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'shelling' & technolo_1 %in% big_arms)), by = list(date)]
data[isr_event > 0, isr_bigshelling := isr_bigshelling / isr_event, by = list(date)]

# Bombings
data[, pal_bombing := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'bombing')), by = list(date)]
data[pal_event > 0, pal_bombing := pal_bombing / pal_event, by = list(date)]
data[, pal_suicidebombing := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'bombing' & (context %in% 'suicide' | technolo_1 %in% 'belt'))), by = list(date)]
data[pal_event > 0, pal_suicidebombing := pal_suicidebombing / pal_event, by = list(date)]
data[, isr_bombing := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'bombing')), by = list(date)]
data[isr_event > 0, isr_bombing := isr_bombing / isr_event, by = list(date)]

# Raids
data[, pal_raid := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'raid')), by = list(date)]
data[pal_event > 0, pal_raid := pal_raid / pal_event, by = list(date)]
data[, isr_raid := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'raid')), by = list(date)]
data[isr_event > 0, isr_raid := isr_raid / isr_event, by = list(date)]
data[, isr_smallraid := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'raid' & technolo_1 %in% small_arms)), by = list(date)]
data[isr_event > 0, isr_smallraid := isr_smallraid / isr_event, by = list(date)]
data[, isr_bigraid := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'raid' & technolo_1 %in% big_arms)), by = list(date)]
data[isr_event > 0, isr_bigraid := isr_bigraid / isr_event, by = list(date)]

# Beatings 
data[, pal_beating := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'beating')), by = list(date)]
data[pal_event > 0, pal_beating := pal_beating / pal_event, by = list(date)]

# Clashes
data[, pal_clash := as.numeric(sum(actor1 %in% pal_milactors & interactio == 'crowd control')), by = list(date)]
data[pal_event > 0, pal_clash := pal_clash / pal_event, by = list(date)]

# Air strikes
data[, isr_airstrike := as.numeric(sum(actor1 %in% isr_milactors & interactio == 'air strike')), by = list(date)]
data[isr_event > 0, isr_airstrike := isr_airstrike / isr_event, by = list(date)]

# Nonviolent/defensive
data[, isr_nonviol := as.numeric(sum(actor1 %in% isr_milactors & interactio %in% c('bulldozing', 'detainment', 'movement restriction'))), by = list(date)]
data[isr_event > 0, isr_nonviol := isr_nonviol / isr_event, by = list(date)]

### Type of technology
# Heavy vs light arms
data[, pal_bigtech := as.numeric(sum(actor1 %in% pal_milactors & technology %in% big_arms)), by = list(date)]
data[, pal_smalltech := as.numeric(sum(actor1 %in% pal_milactors & technology %in% small_arms)), by = list(date)]
data[pal_event > 0, pal_bigtech := pal_bigtech / pal_event, by = list(date)]
data[, isr_bigtech := as.numeric(sum(actor1 %in% isr_milactors & technology %in% big_arms)), by = list(date)]
data[, isr_smalltech := as.numeric(sum(actor1 %in% isr_milactors & technology %in% small_arms)), by = list(date)]
data[isr_event > 0, isr_bigtech := isr_bigtech / isr_event, by = list(date)]

# Direct vs indirect fire
data[, pal_direct := as.numeric(sum((actor1 %in% pal_milactors & technology %in% direct_fire) | (interactio %in% 'firefight'))), by = list(date)]
data[pal_event > 0, pal_direct := pal_direct / pal_event, by = list(date)]
data[, pal_indirect := as.numeric(sum(actor1 %in% pal_milactors & technology %in% indirect_fire)), by = list(date)]
data[pal_event > 0, pal_indirect := pal_indirect / pal_event, by = list(date)]
data[, isr_direct := as.numeric(sum((actor1 %in% isr_milactors & technology %in% direct_fire) | (interactio %in% 'firefight'))), by = list(date)]
data[isr_event > 0, isr_direct := isr_direct / isr_event, by = list(date)]
data[, isr_indirect := as.numeric(sum(actor1 %in% isr_milactors & technology %in% indirect_fire)), by = list(date)]
data[isr_event > 0, isr_indirect := isr_indirect / isr_event, by = list(date)]
...
data[, pal_remote := as.numeric(sum(actor1 %in% pal_milactors 
                                    ...
                                    & israel == 1)), by = list(date)]
data[pal_event > 0, pal_remote := pal_remote / pal_event, by = list(date)]
plot(data$pal_remote)


### Targets
data[, pal_civtargeting := as.numeric(sum(actor1 %in% pal_milactors & actor2 %in% isr_civactors & interactio %in% violent_events)), by = list(date)]
data[pal_event > 0, pal_civtargeting := pal_civtargeting / pal_event, by = list(date)]
data[, pal_miltargeting := as.numeric(sum(actor1 %in% pal_milactors & actor2 %in% isr_milactors & interactio %in% violent_events)), by = list(date)]
data[pal_event > 0, pal_miltargeting := pal_miltargeting / pal_event, by = list(date)]

data[, isr_civtargeting := as.numeric(sum(actor1 %in% isr_milactors & actor2 %in% pal_civactors 
                                          & interactio %in% violent_events & interactio != 'raid')), by = list(date)]
data[isr_event > 0, isr_civtargeting := isr_civtargeting / isr_event, by = list(date)]
data[, isr_miltargeting := as.numeric(sum(actor1 %in% isr_milactors & actor2 %in% pal_milactors & interactio %in% violent_events)), by = list(date)]
data[isr_event > 0, isr_miltargeting := isr_miltargeting / isr_event, by = list(date)]
...
bcp_function <- function(inputs){
  # inputs <- scale(inputs)
  bcp_est <- bcp(inputs, w0 = 0.0000001, p0 = 0.0000001, burnin = 1000, mcmc = 10000)
...
var_function <- function(inputs, exog = NULL){
  model1 <- VAR(inputs, exogen = exog, p = 8, ic = 'AIC')
...
response <- 'pal_targ'
irf_m1 <- irf(model1, n.ahead = 8, runs = 1000, response = response, impulse = impulse)
plot(irf_m1)
response <- 'pal_tech'
irf_m1 <- irf(model1, n.ahead = 8, runs = 1000, response = response, impulse = impulse)
plot(irf_m1)
