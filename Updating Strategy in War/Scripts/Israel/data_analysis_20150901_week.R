...
isr_milactors = c('idf')
isr_civactors = c('isrciv', 'isrres', 'isrpol', 'isrgov')
...
# TEST: SEPARATE OUT GAZA VS WEST BANK ACTIONS
# data <- data[(long < 34.75 | israel == 1)]
# data <- data[(long > 34.75 | israel == 1)]
...
response <- 'pal_targ'
irf_m1 <- irf(model1, n.ahead = 12, runs = 1000, response = response, impulse = impulse)
plot(irf_m1)
response <- 'pal_tech'
irf_m1 <- irf(model1, n.ahead = 12, runs = 1000, response = response, impulse = impulse)
plot(irf_m1)
...
impulse <- c('isr_loc', 'isr_targ', 'isr_tech')
response <- 'pal_targ'
irf_m2 <- irf(model2, n.ahead = 12, runs = 1000, response = response, impulse = impulse)
plot(irf_m2)
