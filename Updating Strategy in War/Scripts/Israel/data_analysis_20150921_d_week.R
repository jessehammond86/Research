...
dev.off()
pdf(file = 'IsrPalEvents.pdf')
plot.zoo(isr_event, ylim = c(0,70), col = 'blue', xlab = 'Time', ylab = 'Number of events per week (smoothed)')
par(new = T)
plot.zoo(pal_event, ylim = c(0,70), col = 'green', bty = 'n', xlab = '', ylab = '')
...
#####################################################################################
#### ENDOGENOUS SYSTEMS
exog = cbind(lag(diff(xts(input_data[,list(pal_casratio)], order.by = input_data$week))),
             lag(diff(xts(input_data[,list(pal_casratio)], order.by = input_data$week)), 2)
             , lag(diff(xts(input_data[,list(pal_casratio)], order.by = input_data$week)), 3))
exog[is.na(exog)] <- 0
...
## ISR ACT UP - ISR TARG DOWN - PAL ACT DOWN
