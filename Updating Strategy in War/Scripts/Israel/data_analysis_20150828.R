...
# Firefights
data[, pal_firefight := as.numeric(sum(interactio == 'firefight')), by = week]
# data[pal_event > 0, pal_firefight := pal_firefight / pal_event, by = week]
data[, isr_firefight := as.numeric(sum(interactio == 'firefight')), by = week]
...
data[, pal_direct := as.numeric(sum((actor1 %in% pal_milactors & technology %in% direct_fire) | interactio %in% 'firefight')), by = week]
...
data[, isr_direct := as.numeric(sum((actor1 %in% isr_milactors & technology %in% direct_fire) | interactio %in% 'firefight')), by = week]
...
pal_casr_dt <- data.table(matrix(inputs4))
