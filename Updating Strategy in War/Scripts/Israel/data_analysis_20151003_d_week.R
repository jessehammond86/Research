...
armodel1 <- arima(input_data[, list(isr_bigtech)], order = c(3,0,0), xreg = exog)
armodel1
...
inputs = xts(input_data[,list(pal_event, isr_smalltech
)], order.by = input_data$week)
...
inputs = xts(input_data[,list(pal_event2, isr_smalltech
)], order.by = input_data$week)
# Using VECM model because ISR small tech is not stationary
model1 <- VECM(inputs, lag = 4, include = 'both', estim = '2OLS', exogen = as.matrix(exog))
