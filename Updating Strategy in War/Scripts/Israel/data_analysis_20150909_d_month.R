...
### Types of location
# Population
data[, mean_palevent_pop := mean(as.numeric(Population[actor1 %in% pal_milactors | interactio %in% 'firefight']), na.rm = T), by = list(date_month)]
data[, sd_palevent_pop := sd(as.numeric(Population[actor1 %in% pal_milactors | interactio %in% 'firefight']), na.rm = T), by = list(date_month)]
...
data[, mean_palevent_pal := mean(palestine[actor1 %in% pal_milactors | interactio %in% 'firefight'], na.rm = T), by = list(date_month)]
...
data[, mean_palevent_bdist := mean(mindist_border[actor1 %in% pal_milactors | interactio %in% 'firefight'], na.rm = T), by = list(date_month)]
data[, sd_palevent_bdist := sd(mindist_border[actor1 %in% pal_milactors | interactio %in% 'firefight'], na.rm = T), by = list(date_month)]
...
data[, pal_remote := as.numeric(sum(actor1 %in% pal_milactors 
                                    & (technology %in% indirect_fire | interactio %in% 'shelling'))), by = list(date_month)]
...
bcp_function <- function(inputs){
  # inputs <- scale(inputs)
  bcp_est <- bcp(inputs, w0 = 0.2, p0 = 0.0025, burnin = 1000, mcmc = 10000)
...
pos_pal_diffloc <- diff(inputs1)
# pos_pal_diffloc <- abs(inputs1)
pos_pal_diffloc[pos_pal_diffloc < 0] <- 0
pos_pal_diffloc[is.na(pos_pal_diffloc)] <- 0
...
neg_pal_diffloc <- diff(inputs1)
neg_pal_diffloc[neg_pal_diffloc > 0] <- 0
neg_pal_diffloc[is.na(neg_pal_diffloc)] <- 0
...
pal_difftech_sd <- sd(pal_difftech)
...
pal_difftarg_sd <- sd(pal_difftarg)
...
# bcp_function <- function(inputs){
#   # inputs <- scale(inputs)
#   bcp_est <- bcp(inputs, w0 = 0.1, p0 = 0.025, burnin = 1000, mcmc = 10000)
#   plot(bcp_est, separated = F)
#   return(bcp_est)
# }
# foo <- SMA(inputs2, n = 3)
# foo <- foo[!is.na(foo)]
# foo2 <- bcp_function(foo)
# plot(foo2, type = 'l')
# 
# foo3 <- foo2$posterior.mean
# foo4 <- foo2$posterior.mean
# plot(foo3, type = 'l', col = 'blue', ylim = c(0,1))
# lines(foo4, col = 'red')
# indata <- data.table('isr' = foo3, 'pal' = foo4)
# fleem <- VAR(indata, p =3)
# summary(fleem)
...
# Negative  shock on PALESTINIAN PROJECTION INTO ISRAEL from ISRAELI PROJECTION INTO PALESTINE 
exog = cbind(lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month)),
             lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month), 2),
...
exog[is.na(exog)] <- 0
model3 <- auto.arima(input_data[, list(pos_pal_loc)], xreg = exog, trace = T)
model3
...
var_function <- function(inputs, exog = NULL){
  model1 <- VAR(inputs, exogen = exog, p = 3, ic = 'AIC', type = 'none')
...
inputs = xts(input_data[,list(pos_pal_tech, pos_isr_tech
)], order.by = input_data$date_month)
exog = cbind(lag(xts(input_data[,list(neg_pal_casr)], order.by = input_data$date_month)), 
             lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month)))
exog[is.na(exog)] <- 0
model2 <- var_function(inputs, exog = exog)
irf_m2 <- irf(model2, n.ahead = 3, runs = 1000)
dev.off()
pdf(file = 'ptech_itech.pdf')
plot(irf_m2)

## PAL TARG UP - ISR TARG UP
inputs = xts(input_data[,list(pos_pal_targ, pos_isr_targ
)], order.by = input_data$date_month)
exog = cbind(lag(xts(input_data[,list(neg_pal_casr)], order.by = input_data$date_month)), 
             lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month)))
exog[is.na(exog)] <- 0
...
irf_m2 <- irf(model2, n.ahead = 3, runs = 1000)
dev.off()
pdf(file = 'ptarg_itarg.pdf')
plot(irf_m2)

### 3-way interactions
## PAL TARG UP - ISR LOC DOWN - PAL TARG DOWN
inputs = xts(input_data[,list(pos_pal_targ, neg_isr_loc, neg_pal_targ
)], order.by = input_data$date_month)
exog = cbind(lag(xts(input_data[,list(neg_pal_casr)], order.by = input_data$date_month)), 
             lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month)))
exog[is.na(exog)] <- 0
model4 <- var_function(inputs, exog = exog)
irf_m4 <- irf(model4, n.ahead = 3, runs = 1000)
dev.off()
pdf(file = 'ptarg_iloc_ptarg.pdf')
plot(irf_m4)
dev.off()

### 4-WAY INTERACTIONS
## PAL TARG UP - ISR TECH UP - ISR TARG UP - PAL TECH UP
inputs = xts(input_data[,list(pos_pal_targ, pos_isr_tech, pos_isr_targ, pos_pal_tech
)], order.by = input_data$date_month)
exog = cbind(lag(xts(input_data[,list(neg_pal_casr)], order.by = input_data$date_month)), 
             lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month)))
exog[is.na(exog)] <- 0
model1 <- var_function(inputs, exog = exog)
irf_m1 <- irf(model1, n.ahead = 3, runs = 1000)
dev.off()
