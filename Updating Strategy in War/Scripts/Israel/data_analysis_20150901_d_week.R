...
### Types of location
# Population
data[, mean_palevent_pop := mean(as.numeric(Population[actor1 %in% pal_milactors]), na.rm = T), by = list(week)]
data[, sd_palevent_pop := sd(as.numeric(Population[actor1 %in% pal_milactors]), na.rm = T), by = list(week)]
...
data[, mean_palevent_pal := mean(palestine[actor1 %in% pal_milactors], na.rm = T), by = list(week)]
...
data[, mean_palevent_bdist := mean(mindist_border[actor1 %in% pal_milactors], na.rm = T), by = list(week)]
data[, sd_palevent_bdist := sd(mindist_border[actor1 %in% pal_milactors], na.rm = T), by = list(week)]
...
bcp_function <- function(inputs){
  # inputs <- scale(inputs)
  bcp_est <- bcp(inputs, w0 = 0.001, p0 = 0.00000001, burnin = 1000, mcmc = 10000)
...
# inputs1 <- xts(data[, list(log(mean_palevent_pop+1))], order.by = data$week)
...
plot(pos_pal_locmeans, type = 'l', col = 'blue')
lines(neg_pal_locmeans, col = 'red')

plot(pos_pal_locprobs, type = 'l', col = 'blue')
lines(neg_pal_locprobs, col = 'red')

# Technology
inputs2 <- SMA(xts(data[, list(pal_indirect)], order.by = data$week), n = Nsmooth)
...
plot(as.vector(inputs2), type = 'l', ylim = c(0,1))
lines(pos_pal_techprobs, col = 'blue')
lines(neg_pal_techprobs, col = 'red')

plot(pos_pal_techmeans, type = 'l', col = 'blue')
lines(neg_pal_techmeans, col = 'red')
...
plot(pos_pal_targmeans, type = 'l', col = 'blue')
lines(neg_pal_targmeans, col = 'red')
...
# inputs1 <- xts(data[, list(mean_isrevent_bdist)], order.by = data$week)
inputs1 <- inputs1[!is.na(inputs1)]
...
plot(pos_isr_locmeans, type = 'l', col = 'blue')
lines(neg_isr_locmeans, col = 'red')

plot(pos_isr_locprobs, type = 'l', col = 'blue')
lines(neg_isr_locprobs, col = 'red')
...
plot(as.vector(inputs2), type = 'l', ylim = c(0,1))
lines(pos_isr_techprobs, col = 'blue')
lines(neg_isr_techprobs, col = 'red')

plot(pos_isr_techmeans, type = 'l', col = 'blue')
lines(neg_isr_techmeans, col = 'red')
...
plot(as.vector(inputs3), type = 'l', ylim = c(0,1))
lines(pos_isr_targprobs, col = 'blue')
lines(neg_isr_targprobs, col = 'red')

plot(pos_isr_targmeans, type = 'l', col = 'blue')
lines(neg_isr_targmeans, col = 'red')

# Casualty Ratio
inputs4 <- EMA(xts(data[, list(pal_casratio)], order.by = data$week), n = Nsmooth)
# inputs4 <- xts(data[, list(pal_casratio)], order.by = data$week)
inputs4 <- inputs4[!is.na(inputs4)]
pal_diffcasr <- diff(inputs3)
pos_pal_diffcasr <- inputs3
...
pos_pal_casrprobs <- pos_pal_casr_bcp$posterior.prob

neg_pal_diffcasr <- inputs3
...
ts_data <- xts(data[, list(pal_casratio)], order.by = data$date)
...
var_function <- function(inputs, exog = NULL){
  model1 <- VAR(inputs, exogen = exog, p = 6, ic = 'AIC')
...
inputs = xts(input_data[,list(pos_pal_targ
                              , pos_isr_loc, pos_isr_targ, pos_isr_tech
#                               , neg_isr_loc, neg_isr_targ, neg_isr_tech
...
## Positive shock on CIVILIAN TARGETING - NEGATIVE
inputs = xts(input_data[,list(pos_pal_targ
#                               , pos_isr_loc, pos_isr_targ, pos_isr_tech
                              , neg_isr_loc, neg_isr_targ, neg_isr_tech
)], order.by = input_data$week)
...
## Negative shock on CIVILIAN TARGETING - POSITIVE
inputs = xts(input_data[,list(neg_pal_targ
                              , pos_isr_loc, pos_isr_targ, pos_isr_tech
#                               , neg_isr_loc, neg_isr_targ, neg_isr_tech
)], order.by = input_data$week)
...
## Positive shock on INDIRECT TACTICS - NEGATIVE
inputs = xts(input_data[,list(pos_pal_tech
#                               , pos_isr_loc, pos_isr_targ, pos_isr_tech
                              , neg_isr_loc, neg_isr_targ, neg_isr_tech
)], order.by = input_data$week)
...
## Negative shock on INDIRECT TACTICS - POSITIVE
inputs = xts(input_data[,list(neg_pal_tech
                              , pos_isr_loc, pos_isr_targ, pos_isr_tech
#                               , neg_isr_loc, neg_isr_targ, neg_isr_tech
)], order.by = input_data$week)
...
inputs = xts(input_data[,list(pos_pal_loc
                              , pos_isr_loc, pos_isr_targ, pos_isr_tech
#                               , neg_isr_loc, neg_isr_targ, neg_isr_tech
)], order.by = input_data$week)
exog = xts(input_data[,list(pos_pal_casr, neg_pal_casr)], order.by = input_data$week)
model2 <- var_function(inputs, exog = exog)
response <- 'pos_pal_loc'
irf_m2 <- irf(model2, n.ahead = 6, runs = 1000, response = response)
plot(irf_m2)
...
inputs = xts(input_data[,list(neg_pal_loc
#                               , pos_isr_loc, pos_isr_targ, pos_isr_tech
                              , neg_isr_loc, neg_isr_targ, neg_isr_tech
)], order.by = input_data$week)
exog = xts(input_data[,list(pos_pal_casr, neg_pal_casr)], order.by = input_data$week)
model2 <- var_function(inputs, exog = exog)
response <- 'neg_pal_loc'
irf_m2 <- irf(model2, n.ahead = 6, runs = 1000, response = response)
plot(irf_m2)
...
inputs = xts(input_data[,list(pos_isr_targ
                              , pos_pal_loc, pos_pal_targ, pos_pal_tech
#                               , neg_pal_loc, neg_pal_targ, neg_pal_tech
)], order.by = input_data$week)
exog = xts(input_data[,list(pos_pal_casr, neg_pal_casr)], order.by = input_data$week)
model2 <- var_function(inputs, exog = exog)
...
#####################################################################################
## Positive shock on LOCATION - POSITIVE
inputs = xts(input_data[,list(pos_isr_loc
                              , pos_pal_loc, pos_pal_targ, pos_pal_tech
                              #                               , neg_pal_loc, neg_pal_targ, neg_pal_tech
)], order.by = input_data$week)
exog = xts(input_data[,list(pos_pal_casr, neg_pal_casr)], order.by = input_data$week)
...
## Negative shock on LOCATION - NEGATIVE
inputs = xts(input_data[,list(neg_isr_loc
#                               , pos_pal_loc, pos_pal_targ, pos_pal_tech
                              , neg_pal_loc, neg_pal_targ, neg_pal_tech
)], order.by = input_data$week)
exog = xts(input_data[,list(pos_pal_casr, neg_pal_casr)], order.by = input_data$week)
