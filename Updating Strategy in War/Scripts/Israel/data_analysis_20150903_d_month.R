...
bcp_function <- function(inputs){
  # inputs <- scale(inputs)
  bcp_est <- bcp(inputs, w0 = 0.00000001, p0 = 0.001, burnin = 1000, mcmc = 10000)
...
plot(as.vector(inputs3), type = 'l', ylim = c(0,1))
lines(pos_isr_locprobs, col = 'blue')
...
#### PAL ACTIONS
exog = lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month))
...
inputs = xts(input_data[,list(neg_isr_targ, neg_isr_loc
                              , pos_pal_targ
)], order.by = input_data$date_month)
model2 <- var_function(inputs, exog = exog)
response <- 'pos_pal_targ'
...
exog = xts(input_data[,list(pos_pal_casr, neg_pal_casr)], order.by = input_data$date_month)
model2 <- var_function(inputs, exog = exog)
...
#####################################################################################
#### ISR ACTIONS
exog = lag(xts(input_data[,list(neg_pal_casr)], order.by = input_data$date_month))
## Positive shock on CIVILIAN TARGETING - POSITIVE
