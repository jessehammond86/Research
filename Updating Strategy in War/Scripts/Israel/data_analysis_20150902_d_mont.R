...
bcp_function <- function(inputs){
  # inputs <- scale(inputs)
  bcp_est <- bcp(inputs, w0 = 0.1, p0 = 0.000000001, burnin = 1000, mcmc = 10000)
...
# Location
inputs1 <- SMA(xts(data[, list(log(mean_palevent_pop+1))], order.by = data$week), n = Nsmooth)
...
pos_pal_diffloc[pal_diffloc < 0] <- -1
...
neg_pal_diffloc[pal_diffloc > 0] <- 1
...
# Technology
inputs2 <- EMA(xts(data[, list(pal_remote)], order.by = data$week), n = Nsmooth)
