...
bcp_function <- function(inputs){
  # inputs <- scale(inputs)
  bcp_est <- bcp(inputs, w0 = 0.001, p0 = 0.001, burnin = 1000, mcmc = 10000)
...
var_function <- function(inputs, exog = NULL){
  model1 <- VAR(inputs, exogen = exog, p = 2, ic = 'AIC')
...
impulse <- c('pos_isr_loc', 'pos_isr_targ', 'pos_isr_tech')
...
#### ISR ACTIONS
## Positive shock on CIVILIAN TARGETING
inputs = xts(input_data[,list(pos_pal_loc, pos_pal_targ, pos_pal_tech
                              , pos_isr_targ
