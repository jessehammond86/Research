...
data[, pal_casratio_w = palcas_ratio * palcas_mo]
data[, isr_casratio_w = isrcas_ratio * isrcas_mo]
### Interactions
data[, event := 1.0]
...
pal_loc <- e.divisive(inputs1, R = 999, sig.lvl = 0.025, min.size = 24)
...
# Technology
inputs2 <- EMA(xts(data[, list(pal_bombing)], order.by = data$week), n = 4)
inputs2 <- inputs2[!is.na(inputs2)]
pal_tech <- e.divisive(inputs2, R = 999, sig.lvl = 0.025, min.size = 24)
pal_tech_dt <- data.table(matrix(inputs2))
setnames(pal_tech_dt, 'pal_bombing')
pal_tech_dt[, regime := pal_tech$cluster]
pal_tech_dt[, regime_mean := mean(pal_bombing), by = regime]
plot(inputs2)
...
pal_targ <- e.divisive(inputs3, R = 999, sig.lvl = 0.025, min.size = 24)
...
inputs1 <- inputs1[!is.na(inputs1)]
isr_loc <- e.divisive(inputs1, R = 999, sig.lvl = 0.025, min.size = 24)
...
inputs2 <- inputs2[!is.na(inputs2)]
isr_tech <- e.divisive(inputs2, R = 999, sig.lvl = 0.025, min.size = 24)
...
inputs3 <- inputs3[!is.na(inputs3)]
isr_targ <- e.divisive(inputs3, R = 999, sig.lvl = 0.025, min.size = 24)
...
inputs3 <- inputs3[!is.na(inputs3)]
pal_casr <- e.divisive(inputs3, R = 999, sig.lvl = 0.025, min.size = 24)
pal_casr_dt <- data.table(matrix(inputs3))
setnames(pal_casr_dt, 'pal_casratio')
pal_casr_dt[, regime := pal_casr$cluster]
pal_casr_dt[, regime_mean := mean(pal_casratio), by = regime]
plot(inputs3)
...
plot(ts_data, ylim = c(0,1))
lines(x = .index(ts_data), y = pal_casr_dt$regime_mean, lty = 1, col = 'red', lwd = 2)
lines(x = .index(ts_data), y = c(isr_tech_dt$regime_mean), lty = 2, col = 'blue', lwd = 2)
