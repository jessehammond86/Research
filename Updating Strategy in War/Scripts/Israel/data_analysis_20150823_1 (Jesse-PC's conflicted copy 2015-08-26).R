...
# Location
# inputs1 <- EMA(xts(data[, list(mean_palevent_pop)], order.by = data$week), n = Nsmooth)
inputs1 <- EMA(xts(data[, list(log(mean_palevent_pop+1))], order.by = data$week), n = Nsmooth)
# inputs1 <- loess(c(data[, list(log(mean_palevent_pop+1))])$V1 ~ c(1:nrow(data)))
...
response <- 'pal_targ'
irf_m1 <- irf(model1, n.ahead = 5, runs = 1000, response = response, impulse = impulse)
plot(irf_m1)
response <- 'pal_tech'
irf_m1 <- irf(model1, n.ahead = 5, runs = 1000, response = response, impulse = impulse)
plot(irf_m1)
...
impulse <- c('isr_loc', 'isr_targ', 'isr_tech')
response <- 'pal_targ'
irf_m2 <- irf(model2, n.ahead = 4, runs = 1000, response = response, impulse = impulse)
plot(irf_m2)
...
impulse <- c('isr_loc', 'isr_targ', 'isr_tech')
response <- 'pal_tech'
irf_m3 <- irf(model3, n.ahead = 4, runs = 1000, response = response, impulse = impulse)
plot(irf_m3)
...
impulse <- c('isr_loc', 'isr_targ', 'isr_tech')
response <- 'pal_loc'
irf_m4 <- irf(model4, n.ahead = 4, runs = 1000, response = response, impulse = impulse)
plot(irf_m4)
