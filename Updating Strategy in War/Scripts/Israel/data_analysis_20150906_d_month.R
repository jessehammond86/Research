...
inputs = xts(input_data[,list(neg_isr_loc, neg_isr_targ
                              , pos_pal_tech
)], order.by = input_data$date_month)
model2 <- var_function(inputs, exog = exog)
response <- 'pos_pal_tech'
...
response <- 'pos_pal_loc'
irf_m2 <- irf(model2, n.ahead = 3, runs = 1000)
plot(irf_m2)xts(input_data[,list(neg_isr_targ
...
#####################################################################################
#### ENDOGENOUS SYSTEMS
inputs = xts(input_data[,list(pos_isr_tech, pos_pal_tech, neg_isr_loc
)], order.by = input_data$date_month)
exog = cbind(lag(xts(input_data[,list(neg_pal_casr)], order.by = input_data$date_month)), 
...
inputs = xts(input_data[,list(neg_pal_loc, neg_isr_loc
)], order.by = input_data$date_month)
exog = cbind(lag(xts(input_data[,list(neg_pal_casr)], order.by = input_data$date_month)), 
             lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month)))
exog[is.na(exog)] <- 0
...
inputs = xts(input_data[,list(pos_pal_targ, pos_isr_tech
)], order.by = input_data$date_month)
exog = cbind(lag(xts(input_data[,list(neg_pal_casr)], order.by = input_data$date_month)), 
             lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month)))
exog[is.na(exog)] <- 0
model3 <- var_function(inputs, exog = exog)
irf_m3 <- irf(model3, n.ahead = 3, runs = 1000)
plot(irf_m3)

inputs = xts(input_data[,list(neg_pal_targ, pos_isr_loc
)], order.by = input_data$date_month)
exog = cbind(lag(xts(input_data[,list(neg_pal_casr)], order.by = input_data$date_month)), 
             lag(xts(input_data[,list(pos_pal_casr)], order.by = input_data$date_month)))
exog[is.na(exog)] <- 0
