...
# data[, isr_smalltech := isr_smalltech * isr_event]
...
inputs = xts(input_data[,list(isr_bigtech, pal_indirect, pal_event
)], order.by = input_data$week)

### CP detection
# PAL indirect
pal_cp <- e.divisive(inputs$pal_indirect, R = 999)
pal_change_idx <- pal_cp$estimates[c(2,3)]

dev.off()
pdf(file = 'palCP.pdf', width = 8, height = 5)
plot(inputs$pal_indirect, main = 'Palestinian Reliance on Indirect-Fire Weapons'
     , ylab = 'Proportion of Events per Week')
abline(v = .index(inputs)[pal_change_idx], col = 'red')
dev.off()
pal_mean_r1 <- mean(inputs$pal_indirect[1:pal_change_idx[1]])
pal_mean_r2 <- mean(inputs$pal_indirect[pal_change_idx[1]:pal_change_idx[2]])
pal_mean_r3 <- mean(inputs$pal_indirect[pal_change_idx[2]:nrow(inputs)])


# ISR heavy arms
isr_cp <- e.divisive(inputs$isr_bigtech, R = 999)
isr_change_idx <- isr_cp$estimates[c(2,3)]

dev.off()
pdf(file = 'isrCP.pdf', width = 8, height = 5)
plot(inputs$isr_bigtech, main = 'Israeli Reliance on Heavy Weaponry'
     , ylab = 'Proportion of Events per Week')
abline(v = .index(inputs)[isr_change_idx], col = 'blue')
dev.off()
isr_mean_r1 <- mean(inputs$isr_bigtech[1:isr_change_idx[1]])
isr_mean_r2 <- mean(inputs$isr_bigtech[isr_change_idx[1]:nrow(inputs)])
