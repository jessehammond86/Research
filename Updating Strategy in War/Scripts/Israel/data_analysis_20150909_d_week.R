...
var_function <- function(inputs, exog = NULL){
  ...
  model1 <- VAR(inputs, exogen = exog, p = 4, ic = 'AIC', type = 'both')
...
irf_m2 <- irf(model2, n.ahead = 8, runs = 1000)
dev.off()
pdf(file = 'ptech_itech.pdf')
plot(irf_m2)

## PAL TARG UP - ISR TARG UP - PAL TARG DOWN - ISR TARG DOWN
inputs = xts(input_data[,list(pos_pal_targ, pos_isr_targ, neg_pal_targ, neg_isr_targ
)], order.by = input_data$week)
exog[is.na(exog)] <- 0
model2 <- var_function(inputs, exog = exog)
irf_m2 <- irf(model2, n.ahead = 8, runs = 1000)
...
irf_m4 <- irf(model4, n.ahead = 8, runs = 1000)
dev.off()
...
irf_m1 <- irf(model1, n.ahead = 8, runs = 1000)
dev.off()
...
## 
inputs = xts(input_data[,list(pos_isr_act, neg_isr_targ, neg_pal_act
)], order.by = input_data$week)
exog[is.na(exog)] <- 0
model1 <- var_function(inputs, exog = exog)
irf_m1 <- irf(model1, n.ahead = 8, runs = 1000)
