...
var_function <- function(inputs, exog, response = NULL){
  model1 <- VAR(inputs, exogen = exog, p = 4, type = 'both')
  print(summary(model1))
  irf_m1 <- irf(model1, response = response, n.ahead = 5)
  plot(irf_m1)
  return(model1)
}
...
start_idx <- 1
stop_idx <- 64
inputs = xts(data[start_idx:stop_idx,list(isrdead_mo, isr_event)], order.by = data$week[start_idx:stop_idx])
exog = xts(data[start_idx:stop_idx,list(pal_event)], order.by = data$week[start_idx:stop_idx])
inputs <- scale(inputs); exog <- scale(exog)
var_function(inputs, exog)
# Period 2
start_idx <- 64
...
# Palestinian casualties and Palestinian actions
# Period 1
start_idx <- 1
stop_idx <- 62
inputs = xts(data[start_idx:stop_idx,list(paldead_mo, pal_event)], order.by = data$week[start_idx:stop_idx])
exog = xts(data[start_idx:stop_idx,list(isr_event)], order.by = data$week[start_idx:stop_idx])
inputs <- scale(inputs); exog <- scale(exog)
var_function(inputs, exog)
# Period 2
start_idx <- 62
...
start_idx <- 1
stop_idx <- 62
inputs = xts(data[start_idx:stop_idx,list(paldead_mo, pal_civtargeting)], order.by = data$week[start_idx:stop_idx])
exog = xts(data[start_idx:stop_idx,list(isr_event, pal_event, isrdead_mo)], order.by = data$week[start_idx:stop_idx])
inputs <- scale(inputs); exog <- scale(exog)
var_function(inputs, exog)
# Period 2
start_idx <- 62
...
# Israeli heavy arms use and Palestinian indirect violence
# Period 1
start_idx <- 1
stop_idx <- 62
inputs = xts(data[start_idx:stop_idx,list(isr_bigtech, pal_indirect)], order.by = data$week[start_idx:stop_idx])
...
exog = xts(data[start_idx:stop_idx,list(isr_event, pal_event, isrdead_mo, paldead_mo)], order.by = data$week[start_idx:stop_idx])
inputs <- scale(inputs); exog <- scale(exog)
var_function(inputs, exog, 'pal_civtargeting')
# Period 2
start_idx <- 62
stop_idx <- 86
inputs = xts(data[start_idx:stop_idx,list(isr_bigtech, pal_civtargeting)], order.by = data$week[start_idx:stop_idx])
