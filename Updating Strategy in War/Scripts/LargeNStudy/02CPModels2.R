...
sum_data <- sum_data[, list(gwno, conflict_id, episode, months, state_changes
...
summary(lm(months ~ (mean_rebel_time - mean_state_time), data = sum_data))
t.test(sum_data[rebel_faster == 0, months], sum_data[rebel_faster == 1, months])


ts_data[, lcensor := 0L]
ts_data[month <= as.Date('1997-01-01'), lcensor := 1L]
test1 <- coxph( Surv(time = months, event = rep(1, nrow(sum_data)), time2 = lcensor, type = 'left') ~
                  (rebel_faster) +
...
# t-test: do conflicts where rebels update MORE OFTEN end in rebel non-defeat?
t.test(sum_data[outcome2 %in% c(-1), rebel_change_ratio], sum_data[outcome2 == 1, rebel_change_ratio])
...
test2 <- coxph( Surv(months, ep_end) ~
                  (rebel_change_time * rebel_changes) + (state_change_time * state_changes) +
